/* @ts-self-types="./axiom_rules_engine_wasm.d.ts" */
 ...
 * @returns {number}
 */
export function artifact_format_version() {
    const ret = wasm.artifact_format_version();
    return ret >>> 0;
}
 ...
 * @param {string} root_target
 * @returns {string}
 */
export function compile(modules_json, root_target) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(modules_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ptr1 = passStringToWasm0(root_target, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
 ...
 * @returns {string}
 */
export function execute(artifact_json, request_json) {
    let deferred4_0;
    let deferred4_1;
    try {
...
function __wbg_get_imports() {
    ...
    return {
        __proto__: null,
        "./axiom_rules_engine_wasm_bg.js": import0,
    };
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        throw new Error('module_or_path is required (explicit wasm URL)'); // vendored patch: keep bundlers from resolving the wasm statically
    }
    const imports = __wbg_get_imports();
